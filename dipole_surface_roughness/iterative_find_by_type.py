...
parser.add_argument('-i','--iter',type=int, default=1, help='# of iterations')
...
for n in range(args.iter):
    for i in range(frame.shape[0]):
        ...
        neigh_dipoles = dipoles[neighs[:,0]**2 + neighs[:,1]**2 + neighs[:,2]**2 < args.sphere**2]
        neighs = neighs[neighs[:,0]**2 + neighs[:,1]**2 + neighs[:,2]**2 < args.sphere**2,:]
        if args.weighted:
            if firstpass:
                dipole = np.sum(neighs*weight(neighs,*weightParams), axis=0)
            else:
                dipole = np.sum((neighs*weight(neighs,*weightParams)
                                 /(1.0-np.sqrt(np.tile(neigh_dipoles,(3,1)).T))), 
                                axis=0)
        else:
